...
//  
//
//  Created by Vishal on 2024/05/26.
//
...
/// `XCamOption` allows customization of various aspects of the video recording process,
/// such as the video asset configuration, session settings and logging preferences.
public struct XCamOption {
    ...
    /// `Session` configuration object which holds the settings to be applied for the `XCam` session,
    ...
    /// Creates an `XCamOption` with specified album name and an option to enable logging.
    ///
    /// - Parameters:
    ...
    /// Creates an `XCamOption` with specified asset, session and log options.
    ///
    /// - Parameters:
...
public extension XCamOption {
    ...
    struct Asset {
        ...
        init(
            ...
            synchronizeWithLocalAlbum: Bool = false,
...
/// `XCamOption` extension related to file naming rules and file extensions.
public extension XCamOption {
