...
public extension [VideoAsset] {
    ...
    ///
    /// If your goal is simply to keep `VideoAssets` up to date, consider using the `fetchVideoFiles` of `XCamSession`.
...
public extension [PhotoAsset] {
    ...
    /// The `id` of a `VideoAsset` is the `id` of the encapsulated `PHAsset`, and this method operates based on that.
    ///
    /// If your goal is simply to keep `PhotoAsset` up to date, consider using the `fetchPhotoFiles` of `XCamSession`.
