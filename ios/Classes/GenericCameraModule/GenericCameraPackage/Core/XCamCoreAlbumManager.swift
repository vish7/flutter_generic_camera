...
class XCamCoreAlbumManager: NSObject {
    // Dependencies
    ...
    func run<T: XCamAssetProcessing>(processor: T) async throws {
        guard
            case .authorized = await photoLibrary.requestAuthorization(for: .addOnly)
        else {
            let error = XCamError.album(reason: .unabledToAccess)
    ...
    func run<T: XCamAssetLoading>(loader: T) async throws -> T.ReturnType {
        guard
...
extension XCamCoreAlbumManager {
...
extension XCamCoreAlbumManager: PHPhotoLibraryChangeObserver {
