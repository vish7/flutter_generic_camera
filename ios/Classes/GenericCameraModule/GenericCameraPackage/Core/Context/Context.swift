...
//  
//
//  Created by Vishal on 2024/06/24.
//
...
public protocol CommonContext {
    ...
    ///
    /// - Returns: The instance of `XCamSession`, allowing for method chaining.
    ...
    ///
    /// - Returns: `XCamVideoContext`, for chaining calls.
    ...
    ///
    /// It refers to `XCamOption.Session.cameraDevicePreference` when choosing the camera device.
    ///
    /// - Parameters:
    ...
    /// The tuner must conform to `XCamSessionTuning`.
    ///
    /// - Parameters:
    ///   - tuner: An instance that conforms to `XCamSessionTuning`.
    ///   - onComplete: A closure to be executed if the session fails to run the tuner.
    ///
    /// - Returns: `XCamVideoContext`, for chaining calls.
    @discardableResult func custom<T: XCamSessionTuning>(
        _ tuner: T,
        _ onComplete: @escaping CompletionHandler
    ) -> CommonContextType
}
...
public protocol VideoContext {
    ...
    ///
    /// - Returns: The instance of `XCamVideoSessionContext`, allowing for method chaining.
    @discardableResult 
    func video(
...
public protocol PhotoContext {
    ...
    ///
    /// - Returns: The instance of `XCamPhotoContext`, allowing for method chaining.
    @discardableResult func photo(
        _ photoContextOption: PhotoContextOption,
        onComplete: CompletionHandler?
    ) -> PhotoContextType

    /// Sets the flash mode for the camera and returns the updated `XCamPhotoContext` instance.
    ...
    /// - Returns: The updated `XCamPhotoContext` instance.
    @discardableResult func flashMode(to mode: AVCaptureDevice.FlashMode) -> PhotoContextType
    
    /// Sets the red eye reduction mode for the camera and returns the updated `XCamPhotoContext` instance.
    /// The returned instance can be used for chaining configuration.
    ///
    ...
    /// Updates the photo capturing settings for the `XCamPhotoContext` instance.
    ///
...
/// Enum representing various configuration options for an `XCamSession`.
///
...
///
/// It's designed to be used with the `common` method of `XCamSession`
...
public enum CommonContextOption {
    ...
    /// - Parameters:
    ///   - tuner: An instance that conforms to `XCamSessionTuning`.
    case custom(tuner: XCamSessionTuning)
}
...
///
/// It's designed to be used with methods of `XCamSession` to apply these configurations in a streamlined manner.
...
///
/// These options are designed to be used with methods of `XCamSession` to apply configurations effectively.
